...
1. Return to the web browser tab containing your lakehouse, and in the **...** menu for the **Files** node in the **Lake view** pane, select **Upload** and **Upload file**, and then upload the **sales.csv** file from your local computer to the lakehouse.
...
1. In the **...** menu for the **sales.csv** file, select **Properties**. Then copy the **URL** for your file to the clipboard - you will use this later.
...
1. On the **Home** page for the Real-time Analytics experience, select **KQL database** and create a new database with a name of your choice.
1. When the new database has been created, select the option to get data from **OneLake**. Then use the wizard to import the data into a new table by selecting the following options:
    - **Destination**:
...
1. In the ... menu for the **sales** table select **Query table** and then select **Show any 100 records** to open the query workbench editor. The query editor contains some code comments as shown here:

    ```sql
     // Use 'take' to view a sample number of records in the table and check the data.
    // For more details on the take operator, see https://learn.microsoft.com/en-us/azure/data-explorer/kusto/query/takeoperator
    // This query will return 100 records from the table in scope.
    // Take is a simple, quick, and efficient way to view a small sample of records when browsing data interactively.
    // Take command may be considered synonymous to SELECT TOP N FROM TABLE command in SQL.
    // In KQL however, there is no guarantee of which records will be returned unless the source data is sorted.
    // For more details on take operator visit: https://learn.microsoft.com/en-us/azure/data-explorer/kusto/query/takeoperator
    
    sales
    | take 100
    ```

1. Use the **&#9655; Run** button to run the query and review the results, which contain the first 1000 rows of data from the **sales** table.
...
1. Select **Save as query set** and save the query as **Revenue by Product**.
...
1. Select **Build Power BI report** and wait for the report to be generated.
1. Review the preview of the report, then save it as **Sales Revenue By Product.pbix** in your **My Workspace** workspace.
1. At the bottom left of the Power BI portal, select the **Real-time Analytics** icon and switch to the **Power BI** experience.
1. Use the **Workspaces** icon to switch to **My Workspace**.
1. In your workspace, find the **Sales Revenue By Product** dataset.
1. Create a report from the dataset, using the **Auto-create** option, and view the report.
